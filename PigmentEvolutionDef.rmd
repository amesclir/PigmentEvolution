...
simmap.Aurones.Chlorophylls <- makeSimmap(tree=tree, data=Aurones.Chlorophylls, model=pp.Aurones.Chlorophylls.ARDx.2$solution, rate.cat=2, nSim=100, nCores=1)
simmap.Aurones.Chlorophylls <- summary(simmap.Aurones.Chlorophylls)

pdffn = paste0("Model.Aurones.Chlorophylls2rates.ARDx.pdf")
pdf(file=pdffn, width=6, height=8)
plotMKmodel(pp.Aurones.Chlorophylls.ARDx.2)
dev.off()

pdffn = paste0("Aurones.Chlorophylls2rates.ARDx.pdf")
pdf(file=pdffn, width=6, height=60)
plotRECON(tree,pp.Aurones.Chlorophylls.ARDx.2$states,cex=0.5, pie.cex=0.5,title="Aurones.Chlorophylls 2 rates ARDx")
dev.off()
...
cols<-setNames(c("white","lightgrey","darkgrey", "black", "red","darkorange","orange", "yellow"),levels(c("(1,R1)", "(2,R1)", "(3,R1)", "(4,R1)", "(1,R2)", "(2,R2)", "(3,R2)", "(4,R2)")))
pdffn = paste0("Aurones.Chlorophylls2ratesfanARDx.pdf")
pdf(file=pdffn, width=27, height=27)
plot(tree,type="fan",cex=0.6,font=3,lwd=1)
nodelabels(node=1:tree$Nnode+Ntip(tree),
    pie=pp.Aurones.Chlorophylls.ARDx.2$states,piecol=cols,cex=0.2)
dev.off()

cols<-setNames(c("white","yellow","pink", "red", "white","yellow","pink", "red"),levels(c("(1,R1)", "(2,R1)", "(3,R1)", "(4,R1)", "(1,R2)", "(2,R2)", "(3,R2)", "(4,R2)")))
pdffn = paste0("Aurones.Chlorophylls2ratesfanARDx.color.pdf")
pdf(file=pdffn, width=27, height=27)
plot(tree,type="fan",cex=0.6,font=3,lwd=1)
nodelabels(node=1:tree$Nnode+Ntip(tree),
    pie=pp.Aurones.Chlorophylls.ARDx.2$states,piecol=cols,cex=0.2)
...
cols<-setNames(c("white","white","white", "white", "grey","grey","grey", "grey"),levels(c("(1,R1)", "(2,R1)", "(3,R1)", "(4,R1)", "(1,R2)", "(2,R2)", "(3,R2)", "(4,R2)")))
pdffn = paste0("Aurones.Chlorophylls2ratesfanARDx.rates.pdf")
pdf(file=pdffn, width=27, height=27)
plot(tree,type="fan",cex=0.6,font=3,lwd=1)
nodelabels(node=1:tree$Nnode+Ntip(tree),
    pie=pp.Aurones.Chlorophylls.ARDx.2$states,piecol=cols,cex=0.2)
...
```{r}
  ...
  tipstoremove <- setdiff(tree$tip.label,names(vvNPigments))
  tree <- drop.tip(tree, tipstoremove)
...
vvCarotenoids[is.na(vvCarotenoids)]
vvCarotenoids[names(vvCarotenoids)=="Amphilophium_elongatum"] <- 2
vvCarotenoids
...
cols<-setNames(c("#FFFFFF", "#B2E5E5", "#5BC8C8", "#008B8B"),levels(as.factor(vvNPigments)))
colsnode<-setNames(c("#FFFFFF", "#FFFFFF", "#B2E5E5", "#B2E5E5", "#5BC8C8","#5BC8C8", "#008B8B", "#008B8B"),c("(0,R1)", "(0,R2)", "(1,R1)", "(1,R2)", "(2,R1)", "(2,R2)", "(3,R1)", "(3,R2)"))
cols5<-setNames(c("lightyellow","lightgrey"),levels(as.factor(vvFlavonoids)))
cols2<-setNames(c("#e32be3","lightgrey"),levels(as.factor(vvAnthocyanins)))
cols3<-setNames(c("#fa8c00","lightgrey"),levels(as.factor(vvCarotenoids)))
cols4<-setNames(c("#ffe400","lightgrey"),levels(as.factor(vvAurones)))
cols6<-setNames(c("#00dca6","lightgrey"),levels(as.factor(vvChlorophylls)))
node1 <- getMRCA(tree, c("Acritopappus","Richterago_arenaria"))
node2 <- getMRCA(tree, c("Orchis_anthropophora","Gomesa_hydrophila"))
node3 <- getMRCA(tree, c("Clinopodium_nepeta","Stachys_ocymastrum"))
node4 <- getMRCA(tree, c("Pedicularis_sylvatica","Cistanche_phelipaea"))
node5 <- getMRCA(tree, c("Silene_nicaeensis","Paronychia_argentea"))
node6 <- getMRCA(tree, c("Solanum_paniculatum","Nicotiana_obtusifolia"))
node7<- getMRCA(tree, c("Trifolium_fragiferum","Hymenaea"))
node8<- getMRCA(tree, c("Heliotropium_europaeum","Cynoglossum_clandestinum"))
node9<- getMRCA(tree, c("Sisymbrium_austriacum","Marcus-kochia_littorea"))
node10<- getMRCA(tree, c("Malacothamnus_palmeri","Malva_maroccana"))
node11<- getMRCA(tree, c("Sarcocapnos_enneaphylla","Papaver_argemone"))
...
node18<- getMRCA(tree, c("Maurandella_antirrhiniflora","Linaria_spartea"))
node19<- getMRCA(tree, c("Romulea_bulbocodium","Juno_planifolia"))
node20<- getMRCA(tree, c("Allium_roseum","Acis_autumnalis"))
node21<- getMRCA(tree, c("Melomphis_peruviana","Prospero_autumnale"))
node22<- getMRCA(tree, c("Xyris_asperula","Xyris_piranii"))

node35<- getMRCA(tree, c("Aristolochia_paucinervis","Asarum_caudatum"))
node36<- getMRCA(tree, c("Magnolia_biondii","Liriodendron_tulipifera"))
node37<- getMRCA(tree, c("Lilium_pardalinum","Calochortus_albus"))
node38<- getMRCA(tree, c("Angelica_breweri","Scandix_pecten-veneris"))
node39<- getMRCA(tree, c("Erythranthe_moschata","Diplacus_puniceus"))
node40<- getMRCA(tree, c("Arbutus_unedo","Erica_mackaiana"))
node41<- getMRCA(tree, c("Centranthus_calcitrapae","Sambucus_caerulea"))
node42<- getMRCA(tree, c("Ipomoea_granulosa","Calystegia_sepium"))
node43<- getMRCA(tree, c("Fumana_lacidulemiensis","Tuberaria_lignosa"))
node44<- getMRCA(tree, c("Nigella_damascena","Ranunculus_bulbosus"))
node45<- getMRCA(tree, c("Scrophularia_auriculata","Verbascum_sinuatum"))

pdffn = paste0("SM_2R.STEP_NPigments.pdf")
...
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Npigments$ace,piecol=colsnode,cex=0.2)
...
arc.cladelabels(tree, "Orchidaceae", node2, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.5,orientation="curved", stretch=1)
...
arc.cladelabels(tree, "Solan.", node6, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.3,orientation="curved", stretch=1)
...
arc.cladelabels(tree, "Malvaceae", node10, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.3,orientation="curved", stretch=1)
arc.cladelabels(tree, "Papaveraceae", node11, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1,orientation="curved", stretch=1)
...
arc.cladelabels(tree, "Rosaceae", node13, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=2,orientation="curved", stretch=1)
arc.cladelabels(tree, "Malpighiaceae", node14, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1,orientation="curved", stretch=1)
...
arc.cladelabels(tree, "Melastomataceae", node16, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.3,orientation="curved", stretch=1)
...
arc.cladelabels(tree, "Iridaceae", node19, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.5,orientation="curved", stretch=1)
arc.cladelabels(tree, "Amaryllidaceae", node20, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1,orientation="curved", stretch=1)
arc.cladelabels(tree, "Asparagaceae", node21, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1,orientation="curved", stretch=1)
arc.cladelabels(tree, "Xyrid.", node22, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1,orientation="curved", stretch=1)
arc.cladelabels(tree, "Aristoloch.", node35, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1,orientation="curved", stretch=1)
arc.cladelabels(tree, "Magnol.", node36, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1,orientation="curved", stretch=1)
arc.cladelabels(tree, "Liliaceae", node37, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1,orientation="curved", stretch=1)
arc.cladelabels(tree, "Apiaceae", node38, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.3,orientation="curved", stretch=1)
arc.cladelabels(tree, "Phyrm.", node39, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.3,orientation="curved", stretch=1)
arc.cladelabels(tree, "Ericaceae", node40, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.5,orientation="curved", stretch=1)
arc.cladelabels(tree, "Caprifoliaceae", node41, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1,orientation="curved", stretch=1)
arc.cladelabels(tree, "Convolvul.", node42, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.3,orientation="curved", stretch=1)
arc.cladelabels(tree, "Cistaceae", node43, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.3,orientation="curved", stretch=1)
arc.cladelabels(tree, "Ranunculaceae", node44, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.5,orientation="curved", stretch=1)
arc.cladelabels(tree, "Scrophular.", node45, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.3,orientation="curved", stretch=1)
dev.off()
...
colsrates<-setNames(c("#FFC1C1", "#FF4C4C", "#FFC1C1", "#FF4C4C", "#FFC1C1", "#FF4C4C", "#FFC1C1", "#FF4C4C"),c("(0,R1)", "(0,R2)", "(1,R1)", "(1,R2)", "(2,R1)", "(2,R2)", "(3,R1)", "(3,R2)"))
cols5<-setNames(c("lightyellow","lightgrey"),levels(as.factor(vvFlavonoids)))
cols2<-setNames(c("#e32be3","lightgrey"),levels(as.factor(vvAnthocyanins)))
cols3<-setNames(c("#fa8c00","lightgrey"),levels(as.factor(vvCarotenoids)))
cols4<-setNames(c("#ffe400","lightgrey"),levels(as.factor(vvAurones)))
cols6<-setNames(c("#00dca6","lightgrey"),levels(as.factor(vvChlorophylls)))
#Magnolids
node24<- getMRCA(tree, c("Chimonanthus_praecox","Magnolia_sprengeri"))
#Monocots
node25<- getMRCA(tree, c("Arum_italicum","Xyris_piranii"))
#Eudicots
node26<- getMRCA(tree, c("Ranunculus_bulbosus","Solanum_wallacei"))
#Superasteridae
node27<- getMRCA(tree, c("Silene_scabriflora","Lamium_amplexicaule"))
#Asteridae
node28<- getMRCA(tree, c("Cornus_sericea","Lamium_amplexicaule"))
#Lamids
node29<- getMRCA(tree, c("Garrya_elliptica","Lamium_amplexicaule"))
#Campanulids
node30<- getMRCA(tree, c("Oenanthe_globulosa","Achillea_millefolium"))
#Superrodidae
node31<- getMRCA(tree, c("Saxifraga_granulata","Malva_sylvestris"))
#Rodidae
node32<- getMRCA(tree, c("Larrea_tridentata","Malva_sylvestris"))
#Fabids
node33<- getMRCA(tree, c("Larrea_tridentata","Trifolium_subterraneum"))
#Malvids
node34<- getMRCA(tree, c("Geranium_molle","Malva_sylvestris"))


pdffn = paste0("SM_2R.STEP_NPigments_rates.pdf")
pdf(file=pdffn, width=30, height=30)
par(mar = c(0.1, 0.1, 0.1, 0.1))
plot(tree,type="fan",show.tip.label = T, cex = 0.1,  tip.color="white", edge.color = "grey", edge.width = 5, no.margin = F)
tiplabels(pie=summary.simmap.Npigments$tips,piecol=colsrates, bg=colsrates,cex=0.1,offset = 1)
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Npigments$ace,piecol=colsrates,cex=0.2)
add.simmap.legend(colors=colsrates,prompt=FALSE,x=1*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
tiplabels(pie=to.matrix(vvFlavonoids[tree$tip.label],
    levels(as.factor(vvFlavonoids))),piecol=cols5, bg=cols5,cex=0.1, offset = 2.5)
add.simmap.legend(colors=cols5,prompt=FALSE,x=0.9*par()$usr[1],
...
add.simmap.legend(colors=cols4,prompt=FALSE,x=0.7*par()$usr[1],
    y=0.9*par()$usr[3],fsize=2)
arc.cladelabels(tree, "Magnolids", node24, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=2,orientation="curved", stretch=1)
arc.cladelabels(tree, "Monocots", node25, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=4,orientation="curved", stretch=1)
#arc.cladelabels(tree, "Eudicots", node26, ln.offset=1.13, lab.offset=1.23,mark.node=FALSE, cex=2,orientation="curved", stretch=1)
#arc.cladelabels(tree, "Superasterids", node27, ln.offset=1.2, lab.offset=1.3,mark.node=FALSE, cex=1.5,orientation="curved", stretch=1)
#arc.cladelabels(tree, "Asterids", node28, ln.offset=1.11, lab.offset=1.121,mark.node=FALSE, cex=1.5,orientation="curved", stretch=1)
arc.cladelabels(tree, "Lamiids", node29, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=4,orientation="curved", stretch=1)
arc.cladelabels(tree, "Campanulids", node30, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=4,orientation="curved", stretch=1)
#arc.cladelabels(tree, "Superrosids", node31, ln.offset=1.12, lab.offset=1.122,mark.node=FALSE, cex=1.5,orientation="curved", stretch=1)
#arc.cladelabels(tree, "Rosids", node32, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=1.5,orientation="curved", stretch=1)
arc.cladelabels(tree, "Fabids", node33, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=4,orientation="curved", stretch=1)
arc.cladelabels(tree, "Malvids", node34, ln.offset=1.1, lab.offset=1.12,mark.node=FALSE, cex=4,orientation="curved", stretch=1)
dev.off()
```
...
vvAnthocyanins.Carotenoids<-paste(Anthocyanins.Carotenoids[,2],"_",Anthocyanins.Carotenoids[,3],sep="")
names(vvAnthocyanins.Carotenoids)<-Anthocyanins.Carotenoids[,1]
tipstoremove <- setdiff(tree$tip.label,names(vvAnthocyanins.Carotenoids))
tree <- multi2di(mytree$scenario.2)
  tree <- drop.tip(tree, tipstoremove)
...
cols<-setNames(c("lightyellow", "#fa8c00","#e32be3", "#EF5C72"),levels(as.factor(vvAnthocyanins.Carotenoids)))
colsnode<-setNames(c("lightyellow", "lightyellow", "#fa8c00","#fa8c00", "#e32be3", "#e32be3", "#EF5C72", "#EF5C72"),c("(0,R1)", "(0,R2)", "(1,R1)", "(1,R2)", "(2,R1)", "(2,R2)", "(3,R1)", "(3,R2)"))
pdffn = paste0("Figure.Anthocyanins.Carotenoids.pdf")
pdf(file=pdffn, width=30, height=30)
par(mar = c(0.1, 0.1, 0.1, 0.1))
plot(tree,type="fan",show.tip.label = T, cex = 0.1,  tip.color="white", edge.color = "grey", edge.width = 5, no.margin = F)
tiplabels(pie=to.matrix(vvAnthocyanins.Carotenoids[tree$tip.label],
    levels(as.factor(vvAnthocyanins.Carotenoids))),piecol=cols, bg=cols,cex=0.1,offset = 1)
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Anthocyanins.Carotenoids$ace,piecol=colsnode,cex=0.2)
...
pdffn = paste0("Figure.Anthocyanins.Carotenoids_rates.pdf")
pdf(file=pdffn, width=30, height=30)
par(mar = c(0.1, 0.1, 0.1, 0.1))
plot(tree,type="fan",show.tip.label = T, cex = 0.1,  tip.color="white", edge.color = "grey", edge.width = 5, no.margin = F)
tiplabels(pie=summary.simmap.Anthocyanins.Carotenoids$tips,piecol=colsrates, bg=colsrates,cex=0.1,offset = 1)
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Anthocyanins.Carotenoids$ace,piecol=colsrates,cex=0.2)
add.simmap.legend(colors=colsrates,prompt=FALSE,x=1*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
tiplabels(pie=to.matrix(vvFlavonoids[tree$tip.label],
    levels(as.factor(vvFlavonoids))),piecol=cols5, bg=cols5,cex=0.1, offset = 2.5)
...
vvCarotenoids.Chlorophylls<-paste(Carotenoids.Chlorophylls[,2],"_",Carotenoids.Chlorophylls[,3],sep="")
names(vvCarotenoids.Chlorophylls)<-Carotenoids.Chlorophylls[,1]
tipstoremove <- setdiff(tree$tip.label,names(vvCarotenoids.Chlorophylls))
tree <- multi2di(mytree$scenario.2)
  tree <- drop.tip(tree, tipstoremove)
...
cols<-setNames(c("lightyellow", "#00dca6","#fa8c00", "#7DB453"),levels(as.factor(vvCarotenoids.Chlorophylls)))
colsnode<-setNames(c("lightyellow", "lightyellow", "#00dca6","#00dca6", "#fa8c00", "#fa8c00", "#7DB453", "#7DB453"),c("(0,R1)", "(0,R2)", "(1,R1)", "(1,R2)", "(2,R1)", "(2,R2)", "(3,R1)", "(3,R2)"))
pdffn = paste0("Figure.Carotenoids.Chlorophylls.pdf")
pdf(file=pdffn, width=30, height=30)
par(mar = c(0.1, 0.1, 0.1, 0.1))
plot(tree,type="fan",show.tip.label = T, cex = 0.1,  tip.color="white", edge.color = "grey", edge.width = 5, no.margin = F)
tiplabels(pie=to.matrix(vvCarotenoids.Chlorophylls[tree$tip.label],
    levels(as.factor(vvCarotenoids.Chlorophylls))),piecol=cols, bg=cols,cex=0.1,offset = 1)
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Carotenoids.Chlorophylls$ace,piecol=colsnode,cex=0.2)
...
pdffn = paste0("Figure.Carotenoids.Chlorophylls_rates.pdf")
pdf(file=pdffn, width=30, height=30)
par(mar = c(0.1, 0.1, 0.1, 0.1))
plot(tree,type="fan",show.tip.label = T, cex = 0.1,  tip.color="white", edge.color = "grey", edge.width = 5, no.margin = F)
tiplabels(pie=summary.simmap.Carotenoids.Chlorophylls$tips,piecol=colsrates, bg=colsrates,cex=0.1,offset = 1)
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Carotenoids.Chlorophylls$ace,piecol=colsrates,cex=0.2)
add.simmap.legend(colors=colsrates,prompt=FALSE,x=1*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
tiplabels(pie=to.matrix(vvFlavonoids[tree$tip.label],
    levels(as.factor(vvFlavonoids))),piecol=cols5, bg=cols5,cex=0.1, offset = 2.5)
...
tree <- multi2di(mytree$scenario.2)
tree <- drop.tip(tree, tipstoremove)
setdiff(tree$tip.label,names(vAnthocyanins))
setdiff(names(vAnthocyanins),tree$tip.label)
...
cols<-setNames(c("#e32be3","lightyellow"),levels(as.factor(vAnthocyanins)))
colsnode<-setNames(c("#e32be3","#e32be3","lightyellow", "lightyellow"),c("(0,R1)", "(0,R2)", "(1,R1)", "(1,R2)"))
pdffn = paste0("Figure.Anthocyanins.pdf")
pdf(file=pdffn, width=30, height=30)
par(mar = c(0.1, 0.1, 0.1, 0.1))
plot(tree,type="fan",show.tip.label = T, cex = 0.1,  tip.color="white", edge.color = "grey", edge.width = 5, no.margin = F)
tiplabels(pie=to.matrix(vAnthocyanins[tree$tip.label],
    levels(as.factor(vAnthocyanins))),piecol=cols, bg=cols,cex=0.1,offset = 1)
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Anthocyanins$ace,piecol=colsnode,cex=0.2)
...
add.simmap.legend(colors=cols5,prompt=FALSE,x=0.9*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
tiplabels(pie=to.matrix(vAnthocyanins[tree$tip.label],
    levels(as.factor(vAnthocyanins))),piecol=cols2, bg=cols2,cex=0.1, offset = 4)
add.simmap.legend(colors=cols2,prompt=FALSE,x=0.8*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
tiplabels(pie=to.matrix(vvCarotenoids[tree$tip.label],
    levels(as.factor(vvCarotenoids))),piecol=cols3, bg=cols3,cex=0.1, offset = 5.5)
...
colsrates<-setNames(c("#FFC1C1", "#FF4C4C", "#FFC1C1", "#FF4C4C"),c("(0,R1)", "(0,R2)", "(1,R1)", "(1,R2)"))
...
pdffn = paste0("Figure.Anthocyanins_rates.pdf")
pdf(file=pdffn, width=30, height=30)
par(mar = c(0.1, 0.1, 0.1, 0.1))
plot(tree,type="fan",show.tip.label = T, cex = 0.1,  tip.color="white", edge.color = "grey", edge.width = 5, no.margin = F)
tiplabels(pie=summary.simmap.Anthocyanins$tips,piecol=colsrates, bg=colsrates,cex=0.1,offset = 1)
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Anthocyanins$ace,piecol=colsrates,cex=0.2)
add.simmap.legend(colors=colsrates,prompt=FALSE,x=1*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
tiplabels(pie=to.matrix(vvFlavonoids[tree$tip.label],
    levels(as.factor(vvFlavonoids))),piecol=cols5, bg=cols5,cex=0.1, offset = 2.5)
...
tipstoremove <- setdiff(tree$tip.label,names(vAnthocyanins))
tree <- drop.tip(tree, tipstoremove)
setdiff(tree$tip.label,names(vCarotenoids))
setdiff(names(vCarotenoids),tree$tip.label)

vCarotenoids <- vCarotenoids[names(vCarotenoids) != "Loncomelos_narbonensis"]
vCarotenoids <- vCarotenoids[names(vCarotenoids) != "Limniris_macrosiphon"]
vCarotenoids <- vCarotenoids[names(vCarotenoids) != "Thrincia_saxatilis"]
vCarotenoids <- vCarotenoids[names(vCarotenoids) != "Satureja_cuneifolia"]
vCarotenoids <- vCarotenoids[names(vCarotenoids) != "Oxalis_corniculata"]
vCarotenoids <- vCarotenoids[names(vCarotenoids) !=  "Ranunculus_tuberosus"]
...
cols<-setNames(c("#fa8c00","lightyellow"),levels(as.factor(vCarotenoids)))
colsnode<-setNames(c("#fa8c00","#fa8c00","lightyellow", "lightyellow"),c("(0,R1)", "(0,R2)", "(1,R1)", "(1,R2)"))
pdffn = paste0("Figure.Carotenoids.pdf")
pdf(file=pdffn, width=30, height=30)
par(mar = c(0.1, 0.1, 0.1, 0.1))
plot(tree,type="fan",show.tip.label = T, cex = 0.1,  tip.color="white", edge.color = "grey", edge.width = 5, no.margin = F)
tiplabels(pie=to.matrix(vCarotenoids[tree$tip.label],
    levels(as.factor(vCarotenoids))),piecol=cols, bg=cols,cex=0.1,offset = 1)
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Carotenoids$ace,piecol=colsnode,cex=0.2)
...
add.simmap.legend(colors=cols2,prompt=FALSE,x=0.8*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
tiplabels(pie=to.matrix(vCarotenoids[tree$tip.label],
    levels(as.factor(vCarotenoids))),piecol=cols3, bg=cols3,cex=0.1, offset = 5.5)
add.simmap.legend(colors=cols3,prompt=FALSE,x=0.7*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
tiplabels(pie=to.matrix(vvChlorophylls[tree$tip.label],
    levels(as.factor(vvChlorophylls))),piecol=cols6, bg=cols6,cex=0.1, offset = 7)
...
pdffn = paste0("Figure.Carotenoids_rates.pdf")
pdf(file=pdffn, width=30, height=30)
par(mar = c(0.1, 0.1, 0.1, 0.1))
plot(tree,type="fan",show.tip.label = T, cex = 0.1,  tip.color="white", edge.color = "grey", edge.width = 5, no.margin = F)
tiplabels(pie=summary.simmap.Carotenoids$tips,piecol=colsrates, bg=colsrates,cex=0.1,offset = 1)
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=summary.simmap.Carotenoids$ace,piecol=colsrates,cex=0.2)
add.simmap.legend(colors=colsrates,prompt=FALSE,x=1*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
tiplabels(pie=to.matrix(vvFlavonoids[tree$tip.label],
    levels(as.factor(vvFlavonoids))),piecol=cols5, bg=cols5,cex=0.1, offset = 2.5)
